        ...
        <div class="col-md-6 m-5">
            <form action="/search" method="GET" class="search-form">
              <div class="input-group center">
                ...
                <input type="hidden" name="route"  value="{{ $result['route']->id}}">
                <input type="hidden" name="driverscheduleid"  value="{{$result['driverScheduleId']}}">
              
                @endforeach
                <div class="input-group-append">
                    <button type="submit" class="btn btn-dark rounded-right">Search</button>
                </div>
              </div>
            </form>
        </div>
                ...
                <div class="col mb-4">
                    <div class="">
                        <div class="card bg-dark text-white position-relative" style="width:50%">
                            <img style="width:50%" src="{{ asset('images/about-img.png') }}" alt="Background Image"
                                class="mx-auto">

                            <div class="overlay-text">
                                <div class="details mb-3">
                                    <h5 class="card-title">{{ $result['route']->startCity->name }} to
                                        {{ $result['route']->endCity->name }}</h5>
                                ...
                                @auth
                                <form method="post" action="{{ route('confirmReservation' , ['ride' => $result['driverScheduleId']] )}}">
                                    @csrf
                                    <input type="number" class="form-control "  id="number_of_people" name="number_of_people" placeholder="Number Of People" min="1" required>
                          
                                    <div class="card-footer text-center">
                                        <input type="hidden" name="driver_schedule_id" value="{{ $result['driverScheduleId'] }}">
                                        <input type="hidden" name="date" value="{{ $result['scheduleDate'] }}">

                                        <button type="submit" class="btn btn-warning" data-toggle="modal"
                                            data-target="#confirmReservationModal"
                                            data-driver-schedule-id="{{ $result['driverScheduleId'] }}">
                                            Reservation
                                        </button>
                                    </div>
                                </form>
